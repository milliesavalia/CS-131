...
def close_connection(transport):
    transport.close()
    neighbor = transport.get_extra_info('neighbor')
...
def catch_exceptions(loop, context):
    try:
        ...
        if isinstance(exception, ConnectionRefusedError):
            logger.error('Failure to connect, neighboring server may be down')
        else:
...
class ProxyServerClientProtocol(asyncio.Protocol):
    ...
    def __init__(self, server_name):
        ...
        self.floodlist = config.NEIGHBORS[server_name]

    def connection_made(self, transport):
        neighbor = transport.get_extra_info('neighbor')
        logger.info('New connection from {}'.format(neighbor))
    ...
    def data_received(self, data):
        ...
        cmnd = input_list[0]
        args = input_list[1:]
        if (cmnd == 'IAMAT' and self.check_IAMAT(args)):
            response_msg = self.response_IAMAT(input_list[1], input_list[2], input_list[3]) + '\n'
        elif (cmnd == 'WHATSAT' and self.check_WHATSAT(args)):
    ...
    def flood(self, msg):
        counter_list = msg.split()[5:]
        for server_name in self.floodlist:
            if server_name not in counter_list:
                self.propagate(server_name, config.PORT_NUM[server_name], msg)

    def propagate(self, name, port, msg):
        corout = loop.create_connection(lambda: ProxyClientProtocol(msg, name), config.SERVER_HOST, port)
        loop.create_task(corout)

    # Given client_id, returns a lat, lng string tuple
    ...
    # Returns the timestamp of AT stamp
    def parse_stamp_time(self, stamp):
        return float(stamp.split()[5])

    # Update client stamp with new stamp only if new timestamp is greater than old timestamp
    ...
    # Given latlng combined string, returns lat, lng string tuple
    ...
    def response_AT(self, client_id, msg):
        ...
        neighbor = self.transport.get_extra_info('neighbor')
        logger.info('{} connection is server {} propagating data'.format(neighbor, origin_server))
    ...
    def check_WHATSAT(self, args):
        ...
        try:
            ...
            logger.error('Client does not have a location')
            return False
        # Check types
        try:
            radius = float(args[1])
            ...
            logger.error('Radius or bound is of incorrect type')
            return False
        # Check range
        ...
        if bound > 20 or bound < 0:
            logger.error('Bound is out of range')
            return False
        return True
    ...
    def send_WHATSAT(self, client_id, radius_km, info_bound, err_msg):
        ...
        corout = loop.create_connection(protocol, config.API_HOST, config.HTTPS_PORT, ssl=context)
        loop.create_task(corout)
...
if __name__ == '__main__':
    ...
    if not server_name in config.SERVER_LIST:
        ...
        exit(1)
    port_num = config.PORT_NUM[server_name]
    ...
    corout = loop.create_server(lambda: ProxyServerClientProtocol(server_name), config.SERVER_HOST, port_num)
    server = loop.run_until_complete(corout)
